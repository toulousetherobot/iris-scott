...
# Message Settings
MESSAGE_TITLE_FONT = ui.fonts.SF_UI_TEXT_LIGHT_HEADER
MESSAGE_FOOTNOTE_FONT = ui.fonts.SF_UI_TEXT_LIGHT_FOOTNOTE
MESSAGE_TEXT_COLOUR = ui.colours.LIGHT_GRAY
MESSAGE_TITLE_MARGIN_TOP = 100
MESSAGE_FOOTNOTE_RECT = Rect((0, 0, 240, 60)) # , 68)) for 4 lines
MESSAGE_FOOTNOTE_RECT.midtop = (ui.WINDOWWIDTH/2, MESSAGE_TITLE_MARGIN_TOP+30)
...
def message_display(surface, button, title_font=MESSAGE_TITLE_FONT, footnote_font= MESSAGE_FOOTNOTE_FONT, text_color=MESSAGE_TEXT_COLOUR, footnote_text_rect=MESSAGE_FOOTNOTE_RECT):
	Header(surface, "Messages", ui.colours.WHITE)
	...
	icn_img = pygame.image.load(icon_img).convert_alpha()
	icn_img_rect = icn_img.get_rect()
	icn_img_rect.midtop = (ui.WINDOWWIDTH/2, ui.UI_MARGIN_TOP)
	surface.blit(icn_img, icn_img_rect)

	# Header
	button_text_surf = title_font.render(button["title"], True, notification_color)
	button_text_rect = button_text_surf.get_rect()
	button_text_rect.midtop = (ui.WINDOWWIDTH/2, MESSAGE_TITLE_MARGIN_TOP)
	surface.blit(button_text_surf, button_text_rect)
	...
	button_rect.topleft = (ui.UI_MARGIN, footnote_text_rect.y+ footnote_text_rect.height)
	button_rect = FilledRoundedRectangle(surface, button_rect, ui.colours.BUTTON_2_BG_COLOR, 0.4)
	message_buttons.append({"target": button_rect, "value": BUTTON_CLEAR})

	button_text_surf = ui.fonts.SF_UI_DISPLAY_LIGHT.render("Clear", True, ui.colours.BUTTON_2_TEXT_COLOR)
	button_text_rect = button_text_surf.get_rect()
	...
	button_rect = Rect((0,0, 145, 36))
	button_rect.topright = (ui.WINDOWWIDTH - ui.UI_MARGIN, footnote_text_rect.y+ footnote_text_rect.height)
	button_rect = FilledRoundedRectangle(surface, button_rect, ui.colours.BUTTON_3_BG_COLOR, 0.4)
	message_buttons.append({"target": button_rect, "value": BUTTON_ACKNOWLEDGE})


	button_text_surf = ui.fonts.SF_UI_DISPLAY_LIGHT.render("Acknowledge", True, ui.colours.BUTTON_3_TEXT_COLOR)
	button_text_rect = button_text_surf.get_rect()
	button_text_rect.center = button_rect.center
	surface.blit(button_text_surf, button_text_rect)
